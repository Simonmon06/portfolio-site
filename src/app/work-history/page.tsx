        ...
        (
          ...
          <WorkHistoryItem key={index}>
            ...
            <JobHistoryItemDetails>
              ...
              <SkillsContainer>
                {technologies.map((techKey: SkillKey | string, index) => {
                  const skill = SKILLS[techKey as SkillKey];

                  if (!skill) {
                    return null;
                  }

                  const skillKey = String(techKey);
                  const reactKey = `${skillKey}-${index}`;

                  return (
                    <p key={reactKey} className="skill">
                      {skill.icon.normal} {skill.name}
                    </p>
                  );
                })}
              </SkillsContainer>
